...
/** FRONTEND */ ////////////////////////////////////////////////////////////////////////////////////
function blog(){
global $mysqli, $DBprefix, $tema, $page_url, $mod, $tabla, $url_api;
    $tabla = 'blog';
    $vAPI = 'v2';
    $url_api = url_api($vAPI); //echo $url_api;
    $topic = 'topic';
    sql_opciones('link_var', $valor);
    if (!ini_get('allow_url_fopen')) {//die("Error: allow_url_fopen está desactivado en php.ini");
        echo '<div class="col-lg-12 col-xs-12" style="height:100px"><div class="alert alert-info">*allow_url_fopen está desactivado en php.ini.</div></div>' . "\n";
        $i=0;
        ...
        while($row=mysqli_fetch_array($sql)){
            $i++;
                ...
                if ($visible == 1) {
                    ...
                    if ($tema == 'porto' || $tema == 'portophponix') {
                        echo '<!--[' . $i . '] -' . $ID . '-->
                        <div class="col-md-4">
        ...
        if($vAPI == 'v1'){
            $data = query_data1(0, $url_api);
        }else{
            $data1 = query_data1(0, $url_api);//print_r($data1);
            $data = $data1['data'];
        }
        //print_r($data);
        ...
        if ($data != '' && $data != NULL) {
            ...
            if ($ID == '' || $ID == NULL) {
                echo '<div class="col-lg-12 col-xs-12" style="height:100px"><div class="alert alert-info">*Regrese más tarde.</div></div>' . "\n";
            }
        } else {
...
function one_blog(){
global $page_url, $mod, $tabla, $url_api, $id, $back;
    $tabla = table();
