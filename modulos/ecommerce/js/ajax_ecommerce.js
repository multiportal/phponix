// JavaScript Document
let dbAjaxCrud = localStorage.getItem("dbCrud_ecommerce"); //Obtener datos de localStorage
...
function inicio(){
    if (dbAjaxCrud !== null) {
		...
		listar=[];
		listar.push(valor);
		localStorage.setItem("dbCrud_ecommerce", JSON.stringify(listar));
		console.log(listado);		
	}
	dbAjaxCrud = localStorage.getItem("dbCrud_ecommerce");
...
function load(page,q) {
  ...
  $("#loader").fadeIn('slow');
  $.ajax({
    url: 'modulos/ecommerce/admin/backend.php?mod=ecommerce' + action,
    data: parametros,
...
$(document).ready(function () {
  ...
  $(document).on('click', '.activar-listado', function () {	
	...
	localStorage.setItem("dbCrud_ecommerce", JSON.stringify(dbAjaxCrud));
...
imagen5: $("#imagen5").val(),
    ...
    //const url = edit === false ? 'modulos/ecommerce/admin/backend.php?mod=ecommerce&ext=admin/index&action=add' : 'modulos/ecommerce/admin/backend.php?mod=ecommerce&ext=admin/index&action=edit';
    let edo = ($("#id").val() != '') ? 'edit' : 'add';
    const url = 'modulos/ecommerce/admin/backend.php?mod=ecommerce&ext=admin/index&action=' + edo;
  ...
  $(document).on('click', '.btn-edit', function () {
	...
	window.location.href="index.php?mod=ecommerce&ext=admin/index&form=1&action=edit&id="+id;
  });
  ...
  $(document).on('click', '.btn-delete', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.post('modulos/ecommerce/admin/backend.php?action=delete', {
          id
        }, (response) => {
          console.log(response);
          load(1);
  ...
  $(document).on('click', '#Aceptar', function (e) {
    ...
    //console.log(frmData);
    $.ajax({
      url: 'modulos/ecommerce/admin/backend.php?mod=ecommerce&action=subir_cover',
      type: 'POST',
      data: frmData,
      processData: false,
      contentType: false,
...
function imagenes(val) {
  ...
  //console.log(frmData);
  $.ajax({
    url: 'modulos/ecommerce/admin/backend.php?mod=ecommerce&action=subir_cover&val=' + val,
    type: 'POST',
    data: frmData,
    processData: false,
    contentType: false,
...
function refrescar(){
	sessionStorage.removeItem("dbCrud_ecommerce");
	dbAjaxCrud[0] = {
		val: listado
	}
	localStorage.setItem("dbCrud_ecommerce", JSON.stringify(dbAjaxCrud));
