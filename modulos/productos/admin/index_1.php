	...
	switch(true){//switch2/producto/formulario 
		case($action=='form'):
			switch(true){//switch2/producto/formulario/controles
				...
				<div><a href="javascript:up(1);">Subir Imagen</a><div id="upload"></div></div>';
				break;
				case($ctrl=='edit' && !empty($_GET['id'])):
...
//compruebo si las características del archivo son las que deseo 
	if (!((strpos($tipo_archivo, "gif") || strpos($tipo_archivo, "jpeg") || strpos($tipo_archivo, "png")) && ($tamano_archivo < 1000000))) { 
    	...
    	if (@move_uploaded_file($_FILES['userfile']['tmp_name'],$path_archivo)){
			if($_POST['Aceptarima1'] || $ima1){$j=1;}elseif($_POST['Aceptarima2'] || $ima2){$j=2;}else{$j=0;
			...
			<div><a href="javascript:up(1);">Cambiar Imagen</a><div id="upload"></div></div>';
			}
		}else{
...
for($i=1;$i<=5;$i++){
	if($imagen[$i]!=''){$avi='Cambiar';
		$avi1='<i class="fa fa-refresh" aria-hidden="true"></i>';
		$avi2=' | <a href="javascript:borrar('.$i.');" title="Borrar Imagen"><i class="fa fa-trash" aria-hidden="true"></i></a>';
		$img_data='<img src="'.$page_url.'modulos/'.$mod.'/fotos/'.$imagen[$i].'" width="50">';
	}else{$avi='Subir';$avi1='<i class="fa fa-upload" aria-hidden="true"></i>';$avi2='';$img_data='';}
	if($i==$j){
		$input='<input type="hidden" class="form-control" id="ima'.$i.'" name="ima'.$i.'" value="'.$nombre_archivo.'">
		<img src="'.$page_url.'modulos/'.$mod.'/fotos/'.$nombre_archivo.'" style="width:30px;">';
	}else{
		$input='<input type="hidden" class="form-control" id="ima'.$i.'" name="ima'.$i.'" value="'.$imagen[$i].'">';
	}
$img_src.='
<div class="form-group">
	<!--label for="ima'.$i.'">Imagen '.$i.' </label-->
	'.$input.'
	<div><a href="javascript:subir_ima('.$i.');">'.$avi.' Imagen '.$i.'</a><div id="uploadima'.$i.'"></div></div>
	<!--div>'.$img_data.'&nbsp;&nbsp;<a href="#" onClick="subir('.$i.',\''.$imagen[$i].'\');" data-toggle="modal" data-target="#myModal" title="'.$avi.'">'.$avi1.' Imagen '.$i.'</a> '.$avi2.'</div-->
</div>';
}
/*Control de PDFs*/;
$PDF=array('',$pdf1,$pdf2,$pdf3,$pdf4,$pdf5);
for($i=1;$i<=5;$i++){$k++;
	if($PDF[$i]!=''){$avi='Cambiar';
		$avi1='<i class="fa fa-refresh" aria-hidden="true"></i>';
		$avi2=' | <a href="javascript:borrar(-'.$i.');" title="Borrar Archivo"><i class="fa fa-trash" aria-hidden="true"></i></a>';
		$pdf_data='<i class="fa fa-file-pdf-o" aria-hidden="true"></i>';
	}else{$avi='Subir';$avi1='<i class="fa fa-upload" aria-hidden="true"></i>';$avi2='';$pdf_data='';}
$pdf_src.='
<div class="form-group">
	<!--label for="pdf'.$i.'">Archivo PDF '.$i.' </label-->
	<input type="hidden" class="form-control" id="pdf'.$i.'" name="pdf'.$i.'" value="'.$PDF[$i].'">
	<div><a href="javascript:subir_pdf('.$i.');">'.$avi.' Pdf '.$i.'</a><div id="uploadpdf'.$i.'"></div></div>
	<!--div style="color:#f00;">'.$pdf_data.' '.$PDF[$i].'&nbsp;&nbsp;<a href="#" onClick="subir(-'.$i.',\''.$PDF[$i].'\');" data-toggle="modal" data-target="#myModal" title="'.$avi.' Archivo">'.$avi1.' PDF '.$i.'</a> '.$avi2.'</div-->
</div>';
}
...
<div class="col-md-12">
	...
	<div class="col-md-3">
    	<div class="box box-success">
        	...
        	<div class="box-body">
            	<?php //control_cover($cover,$file,$m);
				echo $file;
				?>
        	</div><!-- /.box-body -->
        </div>
