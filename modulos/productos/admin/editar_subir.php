...
if($_POST['Aceptar']){
//datos del arhivo 
if($val!=-1){$repositor='../fotos';$tipos=array('gif','jpeg','png');}else{$repositor='../pdf';$tipos=array('pdf','pdf','pdf');}
...
//compruebo si las características del archivo son las que deseo 
	...
	if(!((strpos($tipo_archivo, $tipos[0]) || strpos($tipo_archivo, $tipos[1]) || strpos($tipo_archivo, $tipos[2])) && ($tamano_archivo < 10000000))) { 
    	...
    	if (@copy($_FILES['userfile']['tmp_name'], $path_file)){//se cambio la funcion #move_uploaded_file" por "copy" para que las imagenes tuvieran los mismos permisos
			...
			$boton='';
			$URL=$page_url.'index.php?mod=productos&ext=admin/index&action=edit&id='.$id;$target='_parent';
