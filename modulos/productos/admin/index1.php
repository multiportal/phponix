		...
		switch(true){
			case($action=='form'):
				switch(true){
					...
					$titulo='Editar';
					$id=$_GET['id'];
					$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."productos_marcas WHERE ID_marca='{$id}';") or print mysqli_error($mysqli); 
...
$visible=$_POST['visible'];
	...
	if($ctrl=='edit'){
		$save=mysqli_query($mysqli,"UPDATE ".$DBprefix."productos_marcas SET logo='{$cover}', nombre='{$nom}', visible='{$visible}' WHERE ID_marca='{$id}';") or print mysqli_error($mysqli);
	}else{
		$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."productos_marcas (logo,nombre,visible) VALUES ('{$cover}','{$nom}','{$visible}')") or print mysqli_error($mysqli);
	}	
	validar_aviso($save,'La Marca se ha guardo correctamente','No se puedo guardar intentelo nuevamente',$aviso);
    	...
    	<?php echo $aviso;?>
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title"><?php echo $titulo;?> Marca</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
...
<script type="text/javascript">
    ...
    var r=confirm("Realmente desea eliminar esta Marca "+id+"?.");
    if(r==true){
	<?php
	print " window.location.href='{$URL}&id='+id+'&delete=1'; ";
	if($_GET['delete']==1 && !empty($_GET['delete'])){
		mysqli_query($mysqli,"DELETE FROM ".$DBprefix."productos_marcas WHERE ID_marca='".$_GET['id']."';") or print mysqli_error($mysqli);
	}
	?>}
    }
</script>  
...
<div class="col-xs-12">
	<div class="box">
    	<div class="box-header">
        	<h3 class="box-title">Tabla Marcas</h3>
...
<?php 
$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."productos_marcas;") or print mysqli_error($mysqli); 
...
echo '
                <tr>
                  <td>'.$id.'</td>
                  <td><img src="'.$page_url.'modulos/productos/fotos/'.$cover.'"  width="80"></td>
				  <td>'.$nom.'</td>
				  <td>'.$visible.'</td>
...
?>
        ...
        <?php 
		$query="SELECT * FROM ".$DBprefix."productos_marcas WHERE visible=1 ORDER BY ID_marca ASC;";
		//crear_json($query:'consulta',$path_f:'ruta del archivo',$nombre_archivo:'nombre_archivo')
		crear_json($query,'modulos/'.$mod.'/','productos_marcas.json');
		?>
	</div><!-- /.col -->
</div><!-- /.col-xs-12 -->
...
$num_rows = mysqli_num_rows($sql);$ord=$num_rows+1;
...
$visible=$_POST['visible'];
	...
	$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."productos_cate (categoria,ord,visible) VALUES ('{$cate}','{$ord}','{$visible}');") or print mysqli_error($mysqli);
	//validar_aviso($save,'mensaje_bien','mensaje_mal',&$aviso)
...
html_iso($nombre,$tit,$cate,$des);
	...
	$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."productos (cover,clave,nombre,titulo,descripcion,resena,precio,moneda,unidad,stock,cate,ID_cate,ID_sub_cate,imagen1,imagen2,imagen3,imagen4,imagen5,alta,visible,user) VALUES ('{$cover}','{$clave}','{$nombre}','{$tit}','{$des}','{$resena}','{$precio}','{$moneda}','{$unidad}','{$stock}','{$cate}','{$cate}','{$subcate}','{$ima1}','{$ima2}','{$ima3}','{$ima4}','{$ima5}','{$alta}','{$visible}','{$username}')") or print mysqli_error($mysqli);
            ...
            <form name="form1" role="form" method="post" enctype="multipart/form-data" action="<?php echo $URL;?>">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="tit" name="tit" value="<?php echo $tit;?>">
                </div>
                <div class="form-group">
...
<script type="text/javascript">
	...
	$num_img=$_GET['img'];
	if($num_img==0){$c_img='cover';}elseif($num_img==-1){$c_img='file';}else{$c_img='imagen'.$num_img;}
            ...
            <form name="form1" role="form" method="post" enctype="multipart/form-data" action="<?php echo $URL;?>">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <input type="hidden" class="form-control" id="pdf" name="pdf" value="<?php echo $pdf;?>">
                  <div>
				  <?php if($pdf!=''){$avi='Cambiar';$avi2=' | <a href="javascript:borrar(-1);">Borrar</a>';?>
                  <img src="<?php echo $page_url.'modulos/'.$mod.'/fotos/pdf_icon.png';?>" width="53px"> <?php echo $pdf;?><br>
				  <?php }else{$avi='Subir';$avi2='';}?>
                  <a href="#" onClick="subir(-1,'<?php echo $pdf;?>');" data-toggle="modal" data-target="#myModal"><?php echo $avi;?> Pdf</a> <?php echo $avi2;?>
                  </div>
                </div>
...
<?php 
$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."productos ORDER BY ID ASC;") or print mysqli_error($mysqli); 
while($reg=mysqli_fetch_array($sql)){
$id		= $reg['ID'];
$nombre = $reg['nombre'];
$cover	= $reg['cover'];
