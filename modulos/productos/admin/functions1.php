<?php 
$replace1=array(' ','.',',','(',')','á','é','í','ó','ú','Á','É','Í','Ó','Ú','&aacute;','&eacute;','&iacute;','&oacute;','&uacute','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;','ñ','Ñ','&ntilde;','&Ntilde;');
$replace2=array('-','-','-','-','-','a','e','i','o','u','A','E','I','O','U','a','e','i','o','u','A','E','I','O','U','n','N','n','N');

function html_iso(&$nombre,&$tit,&$cate,&$des){
global $chartset;
 if($chartset=='iso-8859-1'){
	$nombre=htmlentities($nombre, ENT_COMPAT,'ISO-8859-1', true);
	$tit=htmlentities($tit, ENT_COMPAT,'ISO-8859-1', true);	
...
echo '<select class="form-control" id="cate" name="cate">';
	while($row=mysqli_fetch_array($sql)){
		...
		echo '<option value="'.$row['ID_cate'].'" '.$seleccion.'>'.$row['ord'].' '.$row['categoria'].'</option>';
	}
echo '</select>';	
}
...
<option value="">Seleccionar</option>';
	while($row=mysqli_fetch_array($sql)){
		...
		while($row1=mysqli_fetch_array($sql1)){$categoria=$row1['categoria'];$ord1=$row1['ord'];}
		$seleccion=($row['ID_sub_cate']==$subcate) ? 'selected' : '';
		echo '<option value="'.$row['ID_sub_cate'].'" '.$seleccion.'>'.$ord1.$row['ord'].' '.$row['subcategoria'].'-'.$categoria.'</option>';
	}
echo '</select>';	
}
...
global $mysqli,$DBprefix,$page_url,$mod,$ext,$opc,$idp;	
		$replace=array(' ','.',',','(',')','á','é','í','ó','ú','Á','É','Í','Ó','Ú','&aacute;','&eacute;','&iacute;','&oacute;','&uacute','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;');
		...
		switch(true){
			case ($ext!='' && $ext=='item'):
				...
				if($row=mysqli_fetch_array($sql)){$nom_producto=$row['nombre'];$producto=str_replace($replace,$replace2,$nom_producto);$ID_sub_cate=$row['ID_sub_cate'];}
				...
				if($row=mysqli_fetch_array($sql)){$nom_subcate=$row['subcategoria'];$subcate=str_replace($replace,$replace2,$nom_subcate);$ID_cate=$row['ID_cate'];}

				$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."productos_cate WHERE ID_cate={$ID_cate};") or print mysqli_error($mysqli); 
				if($row=mysqli_fetch_array($sql)){$nom_cate=$row['categoria'];$cate=str_replace($replace,$replace2,$nom_cate);}
...
//MENU
	...
	while($row=mysqli_fetch_array($sql)){$icon=$row['icon'];$ord1=$row['ord'];$categoria=str_replace($replace,$replace2,$row['categoria']);
        ...
        while($row2=mysqli_fetch_array($sql2)){$ord2=$row2['ord'];$sub_cate=ucfirst($row2["subcategoria"]);$subcategoria=str_replace($replace,$replace2,$row2['subcategoria']);
...
global $mysqli,$DBprefix,$tema,$page_url,$path_tema,$mod,$ext,$opc,$tema_previo,$replace1,$replace2;
$replace1=array(' ','.',',','(',')','á','é','í','ó','ú','Á','É','Í','Ó','Ú','&aacute;','&eacute;','&iacute;','&oacute;','&uacute','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;','ñ','Ñ','&ntilde;','&Ntilde;');
$replace2=array('-','-','-','-','-','a','e','i','o','u','A','E','I','O','U','a','e','i','o','u','A','E','I','O','U','n','N','n','N');
 ...
 if($path_JSON){
	...
	if($n!=0){
	 foreach ($Data as $rowm){$i++;
		...
		/*CATEGORIA*******************************************************************/	
		$nomt2='categorias';
		$path_JSON2='modulos/productos/'.$nomt2.'.json';
 		if(!file_exists($path_JSON2)){$path_JSON2=$page_url.'bloques/ws/t/'.$nomt2.'/';}
...
$path_JSON='modulos/productos/'.$fjson;
 ...
 if($path_JSON){
	...
	if($n!=0){
	 foreach ($Data as $rowm){$i++;
		$replace1=array(' ','.',',','(',')','á','é','í','ó','ú','Á','É','Í','Ó','Ú','&aacute;','&eacute;','&iacute;','&oacute;','&uacute','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;');
		$replace2=array('-','-','-','-','-','a','e','i','o','u','A','E','I','O','U','a','e','i','o','u','A','E','I','O','U');
...
echo '<!-- categorias.json -->';
	foreach ($Data as $reg){$i++;
		...
		$replace=array(' ','.',',','(',')','á','é','í','ó','ú','Á','É','Í','Ó','Ú','&aacute;','&eacute;','&iacute;','&oacute;','&uacute','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;');
		$replace2=array('-','-','-','-','-','a','e','i','o','u','A','E','I','O','U','a','e','i','o','u','A','E','I','O','U');
		$categoria=str_replace($replace,$replace2,$reg['categoria']);
...
echo '<!-- subcategoria.json -->';
	foreach ($Data as $reg){$i++;
		...
		$replace=array(' ','.',',','(',')','á','é','í','ó','ú','Á','É','Í','Ó','Ú','&aacute;','&eacute;','&iacute;','&oacute;','&uacute','&Aacute;','&Eacute;','&Iacute;','&Oacute;','&Uacute;');
		$replace2=array('-','-','-','-','-','a','e','i','o','u','A','E','I','O','U','a','e','i','o','u','A','E','I','O','U');
		$subcategoria=str_replace($replace,$replace2,$reg['subcategoria']);
