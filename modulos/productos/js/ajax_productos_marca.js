// JavaScript Document
let dbAjaxCrud = localStorage.getItem("dbCrud_productos_marca"); //Obtener datos de localStorage
...
function inicio(){
    if (dbAjaxCrud !== null) {
		...
		listar=[];
		listar.push(valor);
		localStorage.setItem("dbCrud_productos_marca", JSON.stringify(listar));
		console.log(listado);		
	}
	dbAjaxCrud = localStorage.getItem("dbCrud_productos_marca");
...
function load(page,q) {
  ...
  $("#loader").fadeIn('slow');
  $.ajax({
    url: 'modulos/productos/admin/backend.php?mod=productos&opc=marca' + action,
    data: parametros,
...
$(document).ready(function () {
  ...
  $(document).on('click', '.activar-listado', function () {	
	...
	localStorage.setItem("dbCrud_productos_marca", JSON.stringify(dbAjaxCrud));
...
imagen5: $("#imagen5").val(),
    ...
    //const url = edit === false ? 'modulos/productos/admin/backend.php?mod=productos&ext=admin/index&opc=marca&action=add' : 'modulos/productos/admin/backend.php?mod=productos&ext=admin/index&opc=marca&action=edit';
    let edo = ($("#id").val() != '') ? 'edit' : 'add';
    const url = 'modulos/productos/admin/backend.php?mod=productos&ext=admin/index&opc=marca&action=' + edo;
  ...
  $(document).on('click', '.btn-edit', function () {
	...
	window.location.href="index.php?mod=productos&ext=admin/index&opc=marca&form=1&action=edit&id="+id;
  });
...
function refrescar(){
	sessionStorage.removeItem("dbCrud_productos_marca");
	dbAjaxCrud[0] = {
		val: listado
	}
	localStorage.setItem("dbCrud_productos_marca", JSON.stringify(dbAjaxCrud));
