...
html_iso_porta($nombre,$des,$resena1,$cate);
	...
	$save=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."catalogos (nombre,cover,descripcion,cate,resena,imagen1,imagen2,imagen3,imagen4,imagen5,alta,visible,user) VALUES ('{$nombre}','{$cover}','{$des}','{$cate}','{$resena}','{$ima1}','{$ima2}','{$ima3}','{$ima4}','{$ima5}','{$alta}','{$visible}','{$username}')") or print mysqli_error($mysqli);
	validar_aviso($save,'El catalogos se ha guardo correctamente','No se puedo guardar intentelo nuevamente',$aviso);
    	...
    	<?php echo $aviso;?>
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Agregar Nuevo catalogos</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
...
--><div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" id="resena" name="resena" rows="8"></textarea>
                </div>
                <div class="form-group">
...
<script type="text/javascript">
	...
	$num_img=$_GET['img'];
	if($num_img==0){$c_img='cover';}elseif($num_img<=-1){if($num_img==-1){$num_img='';}$c_img='file'.$num_img;}else{$c_img='imagen'.$num_img;}
	if($_GET['delete']==1 && !empty($_GET['delete'])){mysqli_query($mysqli,"UPDATE ".$DBprefix."catalogos SET ".$c_img."='' WHERE ID='".$_GET['id']."';") or print mysqli_error($mysqli);}
    ?>}
    }
</script>  
<?php 
...
<?php
$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."catalogos WHERE ID='{$id}';") or print mysqli_error($mysqli); 
...
html_iso_porta($nombre,$des,$resena1,$cate);
	...
	$save=mysqli_query($mysqli,"UPDATE ".$DBprefix."catalogos SET nombre='{$nombre}', cover='{$cover}', descripcion='{$des}', cate='{$cate}', resena='{$resena}', imagen1='{$ima1}', imagen2='{$ima2}', imagen3='{$ima3}', imagen4='{$ima4}', imagen5='{$ima5}', alta='{$alta}', visible='{$visible}' WHERE ID='{$id}';") or print mysqli_error($mysqli);
	validar_aviso($save,'El catalogos se ha guardo correctamente','No se puedo guardar intentelo nuevamente',$aviso);
	$URL=$page_url.'index.php?mod='.$mod.'&ext='.$ext;
	recargar(3,$URL,$target);
	}
...
?>
	<div class="col-md-6">
    	<?php echo $aviso;?>
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Editar catalogos</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form name="form1" role="form" method="post" enctype="multipart/form-data" action="<?php echo $URL;?>">
...
--><div class="form-group">
				...
				<?php echo $img_src;?>
                <div class="form-group">
                  <label for="pdf">Archivo PDF 1</label>
...
<script type="text/javascript">
    ...
    var r=confirm("Realmente desea eliminar este catalogos "+id+"?.");
    if(r==true){
	<?php
	print " window.location.href='{$URL}&id='+id+'&delete=1'; ";
	if($_GET['delete']==1 && !empty($_GET['delete'])){
		mysqli_query($mysqli,"DELETE FROM ".$DBprefix."catalogos WHERE id='".$_GET['id']."';") or print mysqli_error($mysqli);
	}
	?>}
    }
</script>  
...
<div class="col-xs-12">
	...
	<div id="desc" class="hidden-xs" style="text-align:center;">Agregar nuevo catalogos</div>
</div>
<?php 
$sql=mysqli_query($mysqli,"SELECT * FROM ".$DBprefix."catalogos ORDER BY ID ASC;") or print mysqli_error($mysqli); 
...
<div class="col-md-12">
        <?php 
		$query="SELECT * FROM ".$DBprefix."catalogos WHERE visible=1 ORDER BY ID ASC;";
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Imagenes de catalogos</h4>
      </div>
      <div class="modal-body">
      	<div><iframe name="ima_subir" frameborder="0" scrolling="auto" width="100%" height="400px" src="<?php echo $page_url.'modulos/catalogos/admin/editar_subir.php';?>"></iframe></div>
      </div>
      <!--
