// JavaScript Document
let dbAjaxCrud = localStorage.getItem("dbCrud_portafolio"); //Obtener datos de localStorage
...
function inicio(){
    if (dbAjaxCrud !== null) {
		...
		listar=[];
		listar.push(valor);
		localStorage.setItem("dbCrud_portafolio", JSON.stringify(listar));
		console.log(listado);		
	}
	dbAjaxCrud = localStorage.getItem("dbCrud_portafolio");
...
function load(page,q) {
  ...
  $("#loader").fadeIn('slow');
  $.ajax({
    url: 'modulos/portafolio/admin/backend.php?mod=portafolio' + action,
    data: parametros,
...
$(document).ready(function () {
  ...
  $(document).on('click', '.activar-listado', function () {	
	...
	localStorage.setItem("dbCrud_portafolio", JSON.stringify(dbAjaxCrud));
...
imagen5: $("#imagen5").val(),
    ...
    //const url = edit === false ? 'modulos/portafolio/admin/backend.php?mod=portafolio&ext=admin/index&action=add' : 'modulos/portafolio/admin/backend.php?mod=portafolio&ext=admin/index&action=edit';
    let edo = ($("#id").val() != '') ? 'edit' : 'add';
    const url = 'modulos/portafolio/admin/backend.php?mod=portafolio&ext=admin/index&action=' + edo;
  ...
  $(document).on('click', '.btn-edit', function () {
	...
	window.location.href="index.php?mod=portafolio&ext=admin/index&form=1&action=edit&id="+id;
  });
  ...
  $(document).on('click', '.btn-delete', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.post('modulos/portafolio/admin/backend.php?action=delete', {
          id
        }, (response) => {
          console.log(response);
          load(1);
  ...
  $(document).on('click', '#Aceptar', function (e) {
    ...
    //console.log(frmData);
    $.ajax({
      url: 'modulos/portafolio/admin/backend.php?mod=portafolio&action=subir_cover',
      type: 'POST',
      data: frmData,
      processData: false,
      contentType: false,
...
function imagenes(val) {
  ...
  //console.log(frmData);
  $.ajax({
    url: 'modulos/portafolio/admin/backend.php?mod=portafolio&action=subir_cover&val=' + val,
    type: 'POST',
    data: frmData,
    processData: false,
    contentType: false,
...
function refrescar(){
	sessionStorage.removeItem("dbCrud_portafolio");
	dbAjaxCrud[0] = {
		val: listado
	}
	localStorage.setItem("dbCrud_portafolio", JSON.stringify(dbAjaxCrud));
