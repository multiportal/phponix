...
const upLoad = (e) => {
    ...
    fetch(CLOUD_URL, data).then(response => response.json())
        .then(res => { console.log(res);
            ...
            //if(img){img.src = (save>0)?page_url+'api/upload/blob.php?id='+data.url:data.url;}
            if (img) {img.src = data.url; /*(data.status == 'Error') ? './../file/images/nodisponible1.jpg' : data.url;*/}
