...
function file_scfg($db_server,$db_username,$db_password,$db_database,$db_database2,$prefijo,$path_f,&$aviso2,&$val){
...
$aviso2=($val==1)?'<div class="alert alert-success" role="alert">Se ha creado el archivo: '.$nombre_archivo.'.</div>':'<div class="alert alert-danger" role="alert">No se ha creado el archivo: '.$nombre_archivo.'. <a class="alert-link" href="javascript:history.go(-1);">Regresar</a></div>';
}
...
$crear_tablas=mysqli_query($mysqli,"CREATE TABLE ".$DBprefix."opciones (ID int(6) unsigned NOT NULL auto_increment,
  ...
  PRIMARY KEY (ID)) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=1;");
//$sql=mysqli_query($mysqli,"INSERT INTO ".$DBprefix."opciones (ID,nom,valor) VALUES ('1','usuarios/login.php');");
