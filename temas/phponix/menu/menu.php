...
if($_SESSION['level']!=-1){echo '<!-- menu.json -->'."\n\r";}else{echo '<!-- menu.json URL:('.$path_JSON.')-->'."\n\r";}
	...
	foreach ($Data as $rowm){$i++;
		...
		if($visible==1 && $sub_menu==NULL){
			...
			echo '<!--'.$i.'-'.$mod_menu.'--><li id="nav-menu-item-'.$ID_menu.'" class="menu-item menu-item-type-custom menu-item-object-custom'.$link_css1.$link_css2.' narrow "><a href="'.$link_url.'" class="">'.$nom_menu.'</a>';
			$n=0;$j=0;
			...
			foreach ($Data as $rowm){
				...
				if($visible2==1 && $sub_menu2==$ID_menu){$j++;
					if($j==1){echo "\n\r".'<div class="popup"><div class="inner" style=""><ul class="sub-menu">'."\n\r";}
					link_url($link_url2,$url_m2,$mod_menu2,$ext_menu2);
					echo '<li id="nav-menu-item-'.$ID_menu.'" class="menu-item menu-item-type-custom menu-item-object-custom " data-cols="1"><!--j:'.$j.'--><a href="'.$link_url2.'">'.$nom_menu2.'</a></li>'."\n\r";
					if($n==$j){echo '</ul></div></div>'."\n\r";}
				}
			}
			echo '</li>';
		}
